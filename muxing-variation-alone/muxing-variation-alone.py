...
fnames = {"num_senders1--100.plot":("Tao 1 - 100",'fig.text(.8, 0.79, "1 - 100", fontsize=20, rotation=-2, color=all_colors["Tao 1 - 100"])'),
          ...
          "num_senderscubicsfqCoDel.plot":("Cubic-over-sfqCoDel",'fig.text(0.65, 0.67, "Cubic-over-sfqCoDel", fontsize=20, rotation=-8, color=all_colors["Cubic-over-sfqCoDel"])'),
          ...
          "num_senderscubic.plot":("Cubic",'fig.text(0.55, 0.56, "Cubic", fontsize=20, rotation=5, color=all_colors["Cubic"])')
          }
...
ax.plot([0, 100], [0, 0], linewidth=3, color="blue")
...
for j, title in enumerate(order):
    ...
    ar, = ax.plot(xvals, yvals, color=colors[j], lw=3, label=title)
    artists.append(ar)
