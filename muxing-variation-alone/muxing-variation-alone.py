...
for i in range(len(dirnames)):
    ...
    colors = plt.get_cmap('Set1')(np.linspace(0, 1.0, 7))
    idx = 0
    for j, (title, data) in enumerate(it):
        alpha = 1.0
        yvals = [normalize(float(d[2]), float(d[1])) for d in data]
        xvals = [float(d[0]) for d in data]
        ar, = ax.plot(xvals, yvals, color=colors[idx], alpha=alpha)
        all_colors[title] = colors[idx]
        if i == 0:
...
fig.text(0.8/2, 0.79, "Tao 1 - 100", fontsize=20, rotation=-2, color=all_colors["Tao 1 - 100"])
fig.text(0.305, 0.745, "Tao 1 - 50", fontsize=20, rotation=-20, color=all_colors["Tao 1 - 50"])
...
fig.text(0.265, 0.31, "Tao 1 - 20", fontsize=20, rotation=-67, color=all_colors["Tao 1 - 20"])
fig.text(0.198, 0.30, "Tao 1 - 10", fontsize=20, rotation=-86, color=all_colors["Tao 1 - 10"])
fig.text(0.153, 0.40, "Tao 1 - 2", fontsize=20, rotation=-87, color=all_colors["Tao 1 - 2"])
